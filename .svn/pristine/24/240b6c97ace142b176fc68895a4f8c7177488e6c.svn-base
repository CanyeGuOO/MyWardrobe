...
- (void)addWardrobe:(NSInteger)tag{
    if (tag == 401) {
        ...
        StickerView *stickerView = [[StickerView alloc] initWithFrame:CGRectMake(_aLong, 10-4, 10, kScreenH-60+8)];
...
#pragma mark -- 删除小挡板
- (void)deleteStickerView{
    for (StickerView *stickerS in self.stickerSAry) {
        [stickerS.subviews lastObject].hidden = ![stickerS.subviews lastObject].hidden;
    }
    for (StickerView *stickerS in self.stickerHAry) {
        [stickerS.subviews lastObject].hidden = ![stickerS.subviews lastObject].hidden;
    }
}
...
- (StickerView *)getHcaseViewWithFrame:(CGRect)rect{
    ...
    stickerView.clipsToBounds = NO;
    UIButton *delBtn = [self getDelBtn:stickerView.frame];
    [stickerView addSubview:delBtn];
    delBtn.hidden = !self.finishBtn.selected;
    return stickerView;
}
#pragma  mark --  生成小橱窗
...
#pragma mark -- 生成删除按钮
- (UIButton *)getDelBtn:(CGRect)rect{
    UIButton *delBtn = [UIButton buttonWithType:(UIButtonTypeCustom)];
    delBtn.frame = CGRectMake(0, 0, 30, 30);
    delBtn.center = CGPointMake(rect.size.width/2, rect.size.height/2);
    [delBtn setImage:[UIImage imageNamed:@"ZDBtn3"] forState:(UIControlStateNormal)];
    delBtn.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    [delBtn addTarget:self action:@selector(selectorDelAction:) forControlEvents:(UIControlEventTouchUpInside)];
    return delBtn;
}
#pragma mark -- 删除
- (void)selectorDelAction:(UIButton *)sender{
    [sender.superview removeFromSuperview];
    StickerView *stickerV = (StickerView *)sender.superview;
    [self.stickerHAry removeObject:stickerV];
    [self.stickerSAry removeObject:stickerV];
...
-(UIButton *)finishBtn{
    if (!_finishBtn) {
        ...
        [_finishBtn setTitle:@"完成" forState:(UIControlStateSelected)];
